...
public class ApproveWorker {
    ...
    @Scheduled(fixedDelayString = "${processing.approve-interval-ms}")
    public void approveSubmittedDocuments() {

        try {
            List<Document> batch = documentRepository.findTopNByStatus(DocumentStatus.SUBMITTED, props.getBatchSize());
            ...
            TransitionRequest request = new TransitionRequest(ids, "system-approve", "auto-approve");
            ...
            log.info("APPROVE-worker: processed={}, success={}, failed={}", results.size(), ok, fail);

        } catch (Exception e) {
            log.error("APPROVE-worker error: {}", e.getMessage(), e);
        }
    }
}
